...
////////////////////////////////////////////////////////////////////////////////
// abc_controller variables
////////////////////////////////////////////////////////////////////////////////
float abc_input[16];
float abc_out[5];
AverageFilterFloat_Size3 kcas_filt;
AverageFilterFloat_Size3 gamma_filt;
AverageFilterFloat_Size3 pitch_filt;
AverageFilterFloat_Size3 pitch_rate_filt;
AverageFilterFloat_Size3 ele_filt;
AverageFilterFloat_Size3 pla_filt;
float gam_in;
float speed;
float V_z;
...
static void update_flight_mode(void)
{
    ...
    case FLY_BY_WIRE_A: {
       /* // set nav_roll and nav_pitch using sticks
        ...
        break;*/
		abc_input[12] = FALSE;  //Predictive
		abc_input[13] = TRUE;  //use ANN
		abc_input[14] = TRUE;  //ABC_engage
		abc_input[15] = 2;      //control mode
		abc_input[16] = FALSE; //Dynamic Inversion
            break;
    }
...
{
	abc_input[0] = (channel_throttle->pwm_to_range()*(24-19)/100)+19;    //scale between 19 and 24 kts	
	abc_input[1] = channel_pitch->pwm_to_angle()/100;
	ahrs.airspeed_estimate(&speed);
	speed        = 0.8*gps.ground_speed_cm()/100.00+0.2*speed;
	gam_in       = safe_asin(V_z/speed); //gamma in rad =asin(V_z/airpseed)	
    abc_input[2] = kcas_filt.apply(speed);
	abc_input[3] = gamma_filt.apply(gam_in);
	abc_input[4] = pitch_filt.apply(ahrs.pitch);                           //pitch in radians
	abc_input[5] = pitch_rate_filt.apply(ins.get_gyro().y);               //pitch rate rad/s
	abc_input[6] = current_loc.alt ;                                     //alt in centimeters msl
	abc_input[7] = ele_filt.apply(channel_pitch->servo_out);             //elevator servo out in centidegrees
	abc_input[8] = pla_filt.apply(channel_throttle->servo_out);          //thr out ranges from 0-100
	
	//convert units
	//abc_input[0] /= 4; 
	abc_input[1] /= 6.4;    //gamma_cmd scaling for +-7 deg
	abc_input[2] *= 1.94384;   //airspeed m/s to kts
	abc_input[3] *= 180/PI; // gamma in deg
	abc_input[4] *= 180/PI; //pitch in deg
	abc_input[5] *= 180/PI; //pitch rate in deg/s
	abc_input[6] *= 0.0328; //alt in ftmsl
	abc_input[7] /= -100;   //ele in deg TE down -> +ve

	if (control_mode!= FLY_BY_WIRE_A && control_mode!= FLY_BY_WIRE_B)
	{
		abc_input[12] = FALSE;  //Predictive
		abc_input[13] = FALSE;  //use ANN
		abc_input[14] = FALSE;  //ABC_engage
		abc_input[15] = 2;      //control mode
		abc_input[16] = FALSE; //Dynamic Inversion
	  
	}	
	abc_controller(abc_input, abc_out);	
	Log_Write_ABC();
...
{
	hal.uartD->print(abc_input[0]);
	hal.uartD->print(",");
	hal.uartD->print(abc_input[2],4);
	hal.uartD->print(",");
	hal.uartD->print(abc_input[1]);
	hal.uartD->print(",");
	hal.uartD->print(abc_input[3]);
	hal.uartD->print(",");
	hal.uartD->print(abc_out[0]);
	hal.uartD->print(",");
	hal.uartD->println(abc_out[1]);
}
